...
═══════════════════════════════════════════════════════════════
MODEL-BASED CONTRACT DESIGN (PREFERRED)
Complete specifications using mathematical models
═══════════════════════════════════════════════════════════════

For container/collection classes, use MML (Mathematical Model Library):
...
2. DEFINE MODEL QUERY
   ...
   feature -- Model (specification layer)
       model: MML_{TYPE} [{PARAMS}]
           -- Abstract mathematical model of this class
           do
...
3. WRITE MODEL-BASED POSTCONDITIONS
   ...
   has (x: G): BOOLEAN
           ...
           model_based: Result = model.has (x)
   ...
   remove (x: G)
           ...
           model_reduced: model |=| old model / x
...
MML TYPE REFERENCE:
| MML Type | Use For | Operations |
|----------|---------|------------|
| MML_SEQUENCE | Ordered, duplicates OK | &, but_last, extended_at |
| MML_SET | Unique, unordered | &, /, has, is_subset |
| MML_BAG | Counted duplicates | &, /, occurrences |
| MML_MAP | Key → Value | extended, removed, item |
| MML_RELATION | Key → Multiple Values | image, inverse |
